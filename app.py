...
if run:
    ...
    try:
        ...
        st.error(f"Could not generate prompts: {e}")
        st.stop()
    ...
    all_rows = []
    engine_tabs = st.tabs(engines)

    for tab, engine in zip(engine_tabs, engines):
        with tab:
            progress = st.progress(0)
            status = st.empty()
            try:
                rows = run_audit(brand.strip(), queries, engine, progress, status)
                all_rows.extend(rows)
            except Exception as e:
                progress.empty()
                status.empty()
                st.error(f"{engine} audit failed: {e}")
    ...
    if not pivot.empty:
        pretty_pivot = pivot.applymap(lambda x: "✅" if x else "❌")
